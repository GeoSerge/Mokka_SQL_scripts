--alter view tableau.v_risk_application as
select
...
-- vc card type info
,case when crh.cash_2_card = 1 and ra.application_status_type_key in (4, 6) and  ra.store_nm like (N'%Мокка — Виртуальная карта%') then 'classic'
	  when ra.application_dt > fst.[first_billing_dt] and ra.application_status_type_key in (4, 6) and  ra.store_nm like (N'%Мокка — Виртуальная карта%') then 'no cash'
	  when ra.application_dt <= fst.[first_billing_dt] and ra.application_status_type_key in (4, 6) and  ra.store_nm like (N'%Мокка — Виртуальная карта%') then  'no billing'
...
-- joining vc_card_type_info
left join cdm.client_risk_hist crh on ra.client_key = crh.client_key and ra.application_dttm between crh.effective_from_dttm and crh.effective_to_dttm
left join (
	select ra.client_key, lb.BillingDate as first_billing_dt
	from risk_DM.risk_application as ra
	left join dbo.LoanBilling lb on lb.LoanKey = ra.loan_key and lb.PaymentNum = 1
	where ra.first_loan_flg = 1 and ra.loan_order_num = 1
	) fst on fst.client_key = ra.client_key
